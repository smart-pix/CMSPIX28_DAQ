...
def generate_plot(npzFile, npySetting, outDir= None):

    print(npySetting)
    inData = np.load(npzFile)
    features = inData["features"]
    # scurve = inData["scurve"]
    info = inspectPath(os.path.dirname(npzFile))
    print(features.shape)
    print(features[0])
    # load the setting file
    # settingsPath = os.path.join(npySetting, "settings.npy")
    settings = np.load(npySetting)
    ...
    fig, ax = plt.subplots()
    sc = ax.scatter(testDelay, testSample, c=mu_c, cmap='viridis', s=256, edgecolor='black')
    ax.set_xlabel('cfg_test_delay')
    ax.set_ylabel('cfg_test_sample')
    ax.set_title('2D Plot of working settings')
    ax.set_aspect('auto')
    fig.colorbar(sc, ax=ax, label='Values')
    fig.tight_layout()
    ...
    if outDir:
        ...
        return fig  # essential to return multiple pages
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Generate 2D plot from input file.')
    parser.add_argument("-i", '--inFilePath', type=str, required=True, help='Input .npz file path')
    parser.add_argument("-o", '--outDir', type=str, default=None, help='Optional output directory')
    args = parser.parse_args()

    generate_plot(args.inFilePath, args.outDir)
...
#         print(iP, inPixPath, pixNum, features[:,pixNum].shape)
#         temp = np.expand_dims(features[:,pixNum], axis=1)
#         generate_plot(temp, inPixPath)
